...
# 공개 API
# ─────────────────────────────────────────────────────────────────────────────
def generate_key(feature: str = "NEURAL", days: int = 365, minutes: int = 0) -> str:
    """
    라이선스 키 발급 (개인키 필요 — 개발자 PC에서만 실행 가능).
    minutes > 0 이면 days 무시하고 분 단위로 만료 설정.
    """
    from cryptography.hazmat.primitives.asymmetric.ec import ECDSA
    from cryptography.hazmat.primitives.hashes import SHA256

    duration_sec = minutes * 60 if minutes > 0 else days * 86400
...
def _cmd_issue(days: int = 0, minutes: int = 0):
    """라이선스 키 발급."""
    try:
        key = generate_key("NEURAL", days=days, minutes=minutes)
        ok, msg = validate_key(key, "NEURAL")
        if minutes > 0:
            print(f"✅ 발급 완료 ({minutes}분)")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--issue-minutes", type=int, metavar="MIN", help="N분짜리 라이선스 발급 (테스트용)")
    parser.add_argument("--verify",   type=str, metavar="KEY",     help="키 유효성 검증")
    args = parser.parse_args()

    if args.gen_key:
        _cmd_gen_key()
    elif args.issue_minutes:
        _cmd_issue(minutes=args.issue_minutes)
    elif args.issue:
        _cmd_issue(days=args.issue)
    elif args.verify:
