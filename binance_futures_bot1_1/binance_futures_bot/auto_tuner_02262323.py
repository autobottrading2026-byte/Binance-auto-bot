...
class AutoTuner:
    def __init__(
        ...
        self.base_clamps = {
            ...
            "watch_limit": (5, 18),  # 최소 5개 심볼 감시
            "max_open_symbols": (2, 12),
    ...
    def classify_regime(self, metrics: Dict[str, float]) -> str:
        ...
        if cand == "trend_up":
            ...
            h.up_hits = 0
            h.down_hits = 0
        if h.up_hits >= self.regime_hits_required:
            h.current_regime = "trend_up"
        elif h.down_hits >= self.regime_hits_required:
            h.current_regime = "trend_down"
        elif h.chop_hits >= self.regime_hits_required:
